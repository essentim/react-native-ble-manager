...
- (instancetype)init
{
    ...
    if (self = [super init]) {
        ...
        readValueCallbacks = [NSMutableDictionary new];
        discoverDescriptorsCallbacks = [NSMutableDictionary new];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error {
    ...
    RCTResponseSenderBlock readCallback = [readValueCallbacks objectForKey:key];
    ...
    if (readCallback != NULL) {
        ...
        readCallback(@[[NSNull null], (([descriptor.value isKindOfClass: [NSData class]]) && ([descriptor.value length] > 0)) ? [descriptor.value toArray] : [NSNull null]]);
        [readValueCallbacks removeObjectForKey:key];
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverDescriptorsForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    NSLog(@"Descriptor Discover");

    NSString *key = [self keyForPeripheral: peripheral andCharacteristic:characteristic];
    RCTResponseSenderBlock readCallback = [discoverDescriptorsCallbacks objectForKey:key];
...
RCT_EXPORT_METHOD(readValue:(NSString *)deviceUUID serviceUUID:(NSString*)serviceUUID  characteristicUUID:(NSString*)characteristicUUID descriptorUUID:(NSString *)descriptorUUID callback:(nonnull RCTResponseSenderBlock)callback)
{
    ...
    if (context) {
        ...
        if (!descriptor) {
            NSString* err = [NSString stringWithFormat:@"Could not find descriptor with UUID %@ on characteristic with UUID %@ on service with UUID %@ on peripheral with UUID %@", descriptorUUID, characteristicUUID,serviceUUID, deviceUUID];
            NSLog(@"Could not find desciptor with UUID %@ on characteristic with UUID %@ on service with UUID %@ on peripheral with UUID %@",
                  descriptorUUID, characteristicUUID,serviceUUID, deviceUUID);
            callback(@[err]);
            return;
        } else {
            NSString *key = [self keyForPeripheral: peripheral andCharacteristic:characteristic andDescriptor:descriptor];
            [readValueCallbacks setObject:callback forKey:key];
...
RCT_EXPORT_METHOD(discoverDescriptors:(NSString *)deviceUUID serviceUUID:(NSString*)serviceUUID  characteristicUUID:(NSString*)characteristicUUID callback:(nonnull RCTResponseSenderBlock)callback)
{
    NSLog(@"discoverDescriptors");
    ...
    if (context) {
        ...
        NSString *key = [self keyForPeripheral: peripheral andCharacteristic:characteristic];
        [discoverDescriptorsCallbacks setObject:callback forKey:key];

        [peripheral discoverDescriptorsForCharacteristic: characteristic]; // callback sends value
    }
...
- (CBDescriptor *) getDescriptor: (CBCharacteristic *)characteristic descriptorUUID:(NSString *)descriptorUUID
{
...
-(NSString *) keyForPeripheral: (CBPeripheral *)peripheral andCharacteristic:(CBCharacteristic *)characteristic andDescriptor:(CBDescriptor *) descriptor {
    return [NSString stringWithFormat:@"%@|%@", [self keyForPeripheral: peripheral andCharacteristic:characteristic], descriptor.UUID.UUIDString];
}
